...
class Buytolet_model extends CI_Model
{
	...
	public function getAptypes($ids)
	{
		...
		// $this->db->group_by('b.type'); // Add group by clause to eliminate duplicates

		$this->db->join('apt_type_tbl as b', 'b.id = a.apartment_type');
	...
	public function getDailyNotificationsCount($userID)
	{
		...
		$this->db->where('status', 0); // add condition to filter by status
